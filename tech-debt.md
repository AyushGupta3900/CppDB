...
| 5 | WHERE supports exactly one `col op literal` condition — no `AND`/`OR`, no `UPDATE` statement. | Keeps the parser a clean single-pass teaching example; `AND` is lexed but unused. | Parse a condition list into a vector of clauses; executor AND-combines predicates. `UPDATE` = parse + read-modify-write under the write lock. | `src/query/parser.cpp`, `src/query/query_executor.cpp` | Open |
| 6 | String literals have no escape sequences (`'O''Brien'` or `\'` unsupported). | Rarely needed for the demo workload. | Handle doubled quotes in `Lexer::tokenize`'s string branch. | `src/query/lexer.cpp` | Open |
